...
**Status:** done - derive tested (incl DST); SSR + jsdom verified

- [x] `derive.contactWindowStatus(client, now)` returns
...
- [x] `derive.nextContactWindow` renders `opensInMinutes` as "opens in Xh Ym".
- [x] A client with no `timezone` shows "timezone not set" and no light.
- [x] The client edit form sets `timezone` from a dropdown of IANA names (US
...
- [x] The client record and each Clients-list row show the client's local time
      ...
      (≤60s).
- [x] Tests: green/amber/red boundaries; `America/Chicago` evaluated in both
