...
**Status:** done - board verified live across all columns + in-build strip

- [x] `derive.conversionColumn(client)` returns one of `check-in | bridge |
...
- [x] The board shows all seven columns in sequence order; only `delivered`
      clients appear on it.
- [x] A `deferred` client sits in Retainer with a visible countdown to
      `do_not_pitch_until`.
- [x] Each card shows business name, client-local time (from ticket 07 if
...
- [x] Clients in phase 1–7 render in a compact "in build" strip, separate from
      the board.
- [x] Clicking a card opens that client's record.
- [x] Tests: every `conversionColumn` branch, including `won`, `closed`,
