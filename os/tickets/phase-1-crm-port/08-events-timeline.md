...
- [ ] The client record shows a timeline of that client's `client_events`,
      newest first, each with its date and `kind`.
- [ ] An add-event control creates an event with a chosen `kind`
      ...
      the picker.
- [ ] An `ascension-signal` event has an open/resolved state (`resolved_at`
      null = open); the record shows a visible flag while any signal is open,
      and a control to resolve it.
- [ ] Timeline entries are never editable or deletable through the UI
      (append-only), matching the `history.md` discipline.
- [ ] Tests: append an event → it appears in `getClient`'s timeline in the
