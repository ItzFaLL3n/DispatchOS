...
- [ ] A migration creates every Phase 1 table with the columns and enum-valued
...
- [ ] `client_events.client_id` cascades on client delete.
- [ ] The `data` module exposes `listClients()` and `getClient(id)` returning
      camelCase objects; the snake↔camel mapper is internal to this module.
- [ ] RLS is off; all access uses the service-role key server-side.
- [ ] The Clients page (behind the gate) lists real rows: business name,
      contact name, phase, retainer status. Empty DB → an empty-state, not an
      error.
- [ ] Test (CRUD boundary, throwaway Supabase schema): insert a client row,
